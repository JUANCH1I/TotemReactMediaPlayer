  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const playlistCanvasRef = canvaMode && dropzoneIndex !== undefined
        ...
        onValue(playlistRef, (snapshot) => {
          ...
          const newPlaylist = data
            ? Object.keys(data).map(key => data[key]).filter((item) => item.videoUrl)
            : [];
    ...
    return () => {
      ...
      if (id) {
        ...
        if (canvaMode && dropzoneIndex !== undefined) {
          const playlistCanvasRef = ref(db, `devices/${id}/playlistCanvas/${dropzoneIndex}`);
